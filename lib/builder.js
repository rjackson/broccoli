...
Builder.prototype.build = function () {
  ...
  /**
    return {tree: instance, totalTime: totalMilliseconds, selfTime: totalMillis, dir: path, subtrees: []};
  */
  return Promise.resolve()
    .then(function () {
      return getReadTreeFn(null, graph)(self.tree) // call self.tree.read()
    })
    .then(function (dir) {
      self.treesRead = newTreesRead
      return { directory: dir, graph: graph }
    }, function (err) {
  ...
  function getReadTreeFn (tree, node) {
    node.tree = tree
    node.subtrees = []
    function readTree (subtree) {
      ...
      if (index === -1) {
        ...
        if (typeof subtree === 'string') {
          childNode.directory = subtree
          childNode.totalTime = 0
          childNode.selfTime  = 0
          childNode.subtrees  = []
          subtreeDir = childNode.tree = subtree
        } else {
          subtreeDir = subtree.read(getReadTreeFn(subtree, childNode))
        }
        return Promise.resolve(subtreeDir)
          .then(function (dir) {
            if (dir == null) throw new Error(subtree + ': .read must return a directory')
            dirsCache[index] = childNode
            return childNode.directory = dir
          })
      } else {
        // Do not re-run .read; just return the cached directory path
        if (dirsCache[index] == null) throw new Error('Tree cycle detected')
        return Promise.resolve(dirsCache[index].directory)
      }
    }
    return readTree
  }
